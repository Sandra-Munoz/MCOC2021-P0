# -*- coding: utf-8 -*-
"""
Created on Wed Aug 18 17:46:46 2021
...
Dts=[]
Mems=[]
fid=open("caso 3 single rendimiento.txt ","w")

for i in range(10):
    Dts=[]
    Mems=[]
    def matriz_laplaciana(N,t=single):
        e=eye(N)-eye(N,N,1)
        return t(e+e.T)

    for N in Ns:
        t1=perf_counter()
        A=matriz_laplaciana(N,t=single)
        t2=perf_counter()
        Am1=inv(A,overwrite_a=False,check_finite=False)
        t3=perf_counter()
